...
<style>
    ...
    /* --- FIN DE ESTILOS DE NOTIFICACIÓN --- */
</style>
';
...
?>
        ...
        <?php display_flash_message(); ?>
                <h1>Panel Principal</h1><p>Bienvenido, <?php echo htmlspecialchars($_SESSION['bibliotecario']['nombre_completo'] ?? ''); ?></p></div>
        </div>
...
async function verificarUsuario(tipo) {
    ...
    try {
        ...
        window.addEventListener('resize', function() { if (window.innerWidth > 768) { sidebar.classList.remove('mobile-open'); sidebarOverlay.classList.remove('active'); } });
        
        // CORRECCIÓN: La lógica de 'active' se maneja por PHP en el header.php
        // Ya no necesitamos este bloque de JS
